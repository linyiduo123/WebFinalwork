...
<body>
    <header class="am-topbar am-topbar-inverse">
        ...
        <ul class="idTabs am-nav am-nav-pills am-topbar-nav">
            <li><a href="main_manager.php">返回</a></li>
        </ul>
  </div>
    ...
    <?php
        ...
        $check = mysqli_query($dbc, "SELECT password FROM manager WHERE id='$username' AND password='$old' limit 1");
        ...
        if(strlen($old)>5 && strlen($new)>5 && strlen($repeat)>5){
            if($old!=$real_password){
                ...
                $query_update = "UPDATE manager SET password='{$new}' WHERE id='{$username}'";
